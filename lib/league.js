...
// console.log('matchups: '+JSON.stringify(data.matchups));
  ...
  if (self.matchups.length<self.schedule.length)
    for (i=self.matchups.length;i<self.schedule.length;i++)
      self.matchups.push(new MatchUp(self.schedule[i]));
...
Season.prototype = {
  ...
  getWeeksMatchup: function() {
    ...
    for (i=0;i<this.matchups.length;i++) {
      var matchDate = this.matchups[i].date;
      ...
      console.log('getWeeksMatchup: '+matchDate);
      if (date===matchDate) {
        console.log('this: '+JSON.stringify(this.matchups[i]));
        return;
