...
=======
    if (!config.localLoad) {
      console.log('Loading League Data from S3');
      s3.getObject(s3_params, function(err, data) {
        if (err) return console.log(err);
        data = JSON.parse(data.Body.toString());
        ...
        // console.log('seasons: '+JSON.stringify(self.seasons));
        console.log('League Data Loaded from S3');
      });
    }
    else {
      console.log('Loading League Data Locally');
      fs.readFile(localSeasonsPath, function read(err, data) {
        if (err) {throw err;}
        data = JSON.parse(data);
        ...
        // console.log('oh: '+JSON.stringify(self.seasons));
        console.log('League Data Loaded Locally');
      });
    }
    ...
    events_.on('season loaded', function() {
      ...
      count++;
        // console.log('count: '+count+' seasons: '+self.seasons.length);
...
<<<<<<< HEAD
    for (i=0;i<self.seasons.length;i++)
      seasonData.seasons.push(self.seasons[i]);
=======
...
<<<<<<< HEAD
      ...
      fs.writeFile(localSeasonsPath,JSON.stringify(self.seasons, null, 4), function (err) {
=======
        console.log('saved: '+JSON.stringify(seasonData));
        console.log('League Data Saved: s3');
      });
    else
...
var Season = function(data) {
  ...
  self.schedule = data.schedule;
  // Build from empty Player list or Schedule
  // add ifSeasonIsCurrent when >1 season data
  if (!self.players||!self.schedule) {
