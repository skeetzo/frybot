...
League.prototype = {
  ...
  load: function(onLoad) {
    ...
    fs.readFile('./lib/seasons.json', function read(err, data) {
      ...
      _.forEach(data.seasons, function (season) {
        this.seasons.push(new Season(season));      
        if (onLoad)
          onLoad();
      });
    });
  ...
  save: function() {
    ...
    return;
    fs.writeFile('./lib/seasons.json',JSON.stringify(this.seasons), function (err) {
      if (err) return console.log(err);
      console.log('League Data Saved');
    });
  },
...
var matchups = [];
  ...
  console.log('Season - '+this.label+' - Loaded');
}
Season.prototype = {
  getLabel: function() {
    return this.label;
