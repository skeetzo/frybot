...
var teamshitPath = './lib/teamshit.json';
var teamshitData;
...
var commands = (function() {
  ...
  (function loadTeamShitData() {
    ...
    fs.readFile(teamshitPath, function read(err, data) {
      if (err) {throw err;}
      teamshitData = JSON.parse(data);
      console.log('Team Shit Loaded');
    });
  })();

  function saveTeamShitData() {
    fs.writeFile(teamshitPath, teamshitData, function (err) {
      if (err) return console.log(err);
      console.log('Team Shit Updated');
    });
  }
  ...
  function scores(argument, message, sender) {
      ...
      confirmedCommand = setTimeout(
        function() {
          if (message.match(statsRegex)!=null) {
            if (config.all_players_.length<=0||all_matches_.length<=0)
    ...
    scores.streak = function (player) {
      ...
      if (typeof player != Object)
        _.forEach(config.all_players_, function (players) {
          if (typeof player != Object)
  ...
  function nicofacts(argument, message, sender) {
    ...
    nicofacts.YES = function() {
      if (nicoFactCounter<=0) {
        ...
        say('You are already subscribed to Nico Facts, bitchass '+sender+'...');
    };
