...
var commands = (function() {
  ...
  */
  ...
  function pregame() {
    ...
    say('Don\'t fuck it up '+players[Math.random(0,players.length)]+'!');
  };
...
exports.activate = function(request) {
  ...
  for (i=0;i<matches.length;i++) 
    if (matches[i]==""||matches[i]==="")
      matches.splice(i,1);
  var command = matches[0].substring(1) || request.command || 'nullo'; // the first command match minus the slash
  var argument = matches[1] || 'nullo'; // the first argument match
  if (argument!='nullo')
