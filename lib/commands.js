  ...
  */
  function bottle(argument, message, sender) {
    ...
    bottle.what = function() {
      ...
      bot.say(bottles[Math.random(0,bottles.length)]);
    };
    if (argument)
      bottle[argument]();
    else
  ...
  */
  function scores(argument, message, sender) {
    ...
    scores.lvp = function() {
      ...
      bot.say('Current LVP: '+leastValuablePlayer.toStats());
    };
    scores.mvp = function() {
      ...
      bot.say('Current MVP: '+mostValuablePlayer.toStats());
    };
    scores.of = function() {
      _.forEach(config.all_players_, function (player) {
      if (player.name==message)
        bot.say('Stats: '+player.toStats());
    });
    };
    scores.streak = function (player) {
      ...
      bot.say(player.name+' is '+streak+' with ('+mod+streakN+')');
    };
    ...
    if (argument)
      ...
      bot.say('What about the scores '+sender+'?');
  };
  this.scores = scores;
  ...
  function suck(argument, message, sender) {
    suck.my = function() {
      ...
      if (sender=='Nico Mendoza'||sender=='Nico')
        bot.say('yeah suck '+sender+'\'s tiny '+message+'!');
      else
        bot.say('yeah suck '+sender+'\'s '+message+'!');
    };
    suck.his = function() {
      bot.say('yeah suck his '+message+'! ');
      bot.say('wait, what?');
    };
    if (argument)
      suck[argument]();
    else
      bot.say('What about sucking '+sender+'\'s '+message+'?');
  };
  this.suck = suck;
})();
