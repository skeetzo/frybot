...
// var this.nicofactsDB = require('./nicofacts.js');
...
commands.prototype = {
  ...
  load : function(callback) {
    ...
    if (!config.localLoad)
      s3.getObject(s3_params, function(err, data) {
        ...
        console.log('Team Shit Loaded: s3');
        onLoad();
      });
    else 
  ...
  */
  bottle : function(argument, message, sender, callback) {
    ...
    if (!this.bottleBitches||this.bottleBitches.length<=0) {
      ...
      if (this.bottleBitches.length<=0) {
        ...
        _.forEach(players, function addTobottleBitches(player) {
          this.bottleBitches.push(player.name);
        });
    ...
    */
    function duty() {
      callback('Bottle Duty: '+self.bottleBitches[0]);
    }
    ...
    */
    function next() {
      ...
      this.saveTeamShitData();
      console.log('bottle duty updated');
    }
    ...
    */
    function what() {
      ...
      callback('Pick up some: '+bottles[Math.random(0,bottles.length)]);
    }
  ...
  scores : function(argument, message, sender, callback) {
    ...
    */
    function lvp() {
      ...
      callback('Current LVP: '+leastValuablePlayer.toStats());
    }
    this.scores.lvp = lvp;
    ...
    */
    function mvp() {
      ...
      callback('Current MVP: '+mostValuablePlayer.toStats());
    }
    this.scores.mvp = mvp;
    ...
    */
    ...
    function streak(player) {
      ...
      callback(player.name+' is '+streak+' with ('+mod+streakN+')');
    }
    ...
    function update() {
      ...
      function sheetReady(err, spreadsheet) {
        if(err) {
          console.log(err);
          setTimeout(function() {
            console.log('retrying sheet load');
            update();
          },5000);
          throw err;
        }
        spreadsheet.receive(function(err, rows, info) {
          ...
          if (message!='quiet')
            callback('Season scores updated');
          else
    ...
    if (argument)
      ...
      callback('What about the scores '+sender+'?');
  },
  ...
  fuck : function(argument, message, sender, callback) {
    ...
    function you() {
      if (message==='frybot'||message==='Frybot') {
        callback('Fuck you '+sender+'!');
        return;
      }
      callback('Yeah fuck you '+message+'!');
    }
    this.fuck.off = you;

    function me() {
      if (!message||(message!='Frybot'&&message!='frybot'))
        callback('I think I\'ll pass..');
      else
        callback('Yeah fuck him '+message+'!');
    }
    this.fuck.off = me;

    function off() {
      if (message==='Frybot'||message==='frybot')
        callback('No you fuck off '+sender+'!');
      else
        callback('Yeah fuck off '+sender+'!');
    }
    this.fuck.off = off;
  ...
  */
  ...
  nicofacts : function(argument, message, sender, callback) {
    ...
    if (sender.indexOf('Nico')>=0) {
      callback('What do you think you\'re doing, bitchass Nico?');
      return;
    }

    var spitNicoFact = function() {
      callback(this.nicofactsDB[this.nicoFactCounter]);
    ...
    function FUCKOFF() {
      if (message!='PLEASE') {
        if (this.nicoFactCounter<10) {
          callback('Invalid response. You have now been permanently subscribed to Nico Facts.');
          if (this.nicoFactPrimed) 
            startNicoFacts();
        }
        else
          callback('Invalid response, motherfucker. Do you even Nico Fact?');
        return;
      }
      callback('You have successfully unsubscribed from Nico Facts.');
      clearInterval(this.nicoFactTimer);
    ...
    function YES() {
      if (this.nicoFactCounter<=0) {
        callback('You have now been subscribed to Nico Facts.');
        startNicoFacts();
      }
      else
        callback('You are all already subscribed to Nico Facts, bitchass '+sender+'...');
    }
    this.YES = YES;

    function NO() {
      callback('Nico Fact #846: No one tells Nico Facts when to stop.');
    }
    this.NO = NO;

    function START() {
      callback('Nico Fact #847: No one tells Nico Facts what to do.');
    }
    this.START = START;
    ...
    if (argument)
      ...
      this.nicoFactCounter = 0;
      callback('Reply \'/nicofacts YES\' to subscribe to Nico Facts for a weekly charge of 1 Jäger Bomb. Standard msg and data rates will apply. You can cancel at anytime by replying \'/nicofacts FUCKOFF\'');
    }
    else {
  ...
  */
  suck : function(argument, message, sender, callback) {
    ...
    function my() {
      ...
      if (sender=='Nico Mendoza'||sender=='Nico')
        callback('yeah suck '+sender+'\'s tiny '+message+'!');
      else
        callback('yeah suck '+sender+'\'s '+message+'!');
    }
    this.my = my;

    function his() {
      callback('yeah suck his '+message+'! ');
      callback('wait, what?');
    }
    this.his = his;
    ...
    if (argument)
      ...
      callback('What about sucking '+sender+'\'s '+message+'?');
  },
  ...
  pregame : function(argument, message, sender, callback) {
    ...
    callback(maybes[Math.round(Math.random()*maybes.length)]);
    callback('Playing @ '+league.getCurrentSeason().getTodaysMatchup().location);
    this.bottle('duty',null,null,callback);
    var players_ = league.getCurrentSeason().getPlayersByNames();
    callback('I\'m betting... '+players_[Math.round(Math.random()*players_.length)]+' gets skunked tonight!');
  }
}
