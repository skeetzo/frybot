...
var commands = function() {
  ...
  // league.events.on('league data loaded', function() {
  //   self.events.emit('commands loaded');
  // });
  ...
  (function loadTeamShitData() {
    ...
    if (!config.localLoad)
      ...
      fs.readFile(localTeamShitPath, function read(err, data) {
        ...
        // events.emit('s3 loaded');
        console.log('Team Shit Loaded: local');
        onLoad();
      });
    function onLoad() {
      league = new League(teamshitData.seasons, function() {
        self.events.emit('commands loaded');
      });
    }
  ...
  */
  this.activate = function(request) {
  ...
  */
  function bottle(argument, message, sender) {
    ...
    if (!config.bottleBitches||config.bottleBitches_.length<=0) {
      config.bottleBitches_ = teamshitData.bottleBitches || [];
      if (config.bottleBitches_.length<=0) {
        console.log('Populating Bottle Duty');
        var players = league.getCurrentSeason().getPlayers();
        ...
        teamshitData.bottleBitches = config.bottleBitches_;
        saveTeamShitData();
      }
      else {
  ...
  */
  ...
  function fuck(argument, message, sender) {
    fuck.you = function() {
      if (message==='frybot'||message==='Frybot') {
        say('Fuck you '+sender+'!');
        return;
      }
      say('Yeah fuck you '+message+'!');
    };
    fuck.me = function() {
      if (!message||(message!='Frybot'&&message!='frybot'))
        say('I think I\'ll pass..');
      else
        say('Yeah fuck him '+message+'!');
    };
    fuck.off = function() {
      if (message==='Frybot'||message==='frybot')
        say('No you fuck off '+sender+'!');
      else
        say('Yeah fuck off '+sender+'!');
    };
    if (argument)
      self.fuck[argument]();
    else
  ...
  */
  ...
  function pregame() {
    ...
    say(maybes[Math.round(Math.random()*maybes.length)]);
    say('Playing @ '+league.getCurrentSeason().getThisWeeksMatchup().location);
    bottle('duty');
    var players_ = league.getCurrentSeason().getPlayersByNames();
    say('I\'m betting... '+players_[Math.round(Math.random()*players_.length)]+' gets skunked tonight!');
  };
