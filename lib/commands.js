...
var commands = function() {
  ...
  */
  commands.prototype.activate = function(request) {
  ...
  */
  function bottle(argument, message, sender) {
      ...
      function sheetReady(err, spreadsheet) {
        ...
        spreadsheet.receive(function(err, rows, info) {
          ...
          spreadsheet.send(function(err) {
            ...
            self.postThought_('Bottle Duty: '+person);
          });
        });
      });
    };
    ...
    bottle.what = function() {
      ...
      self.postThought_(bottles[Math.random(0,bottles.length)]);
    };
    if (argument)
      bottle[argument]();
    else
      self.postThought_('bottle fail');
  };
  this.bottle = bottle;

  // Runs the cool guy thing
  function coolguy() {
    self.postThought_(cool());
  };
  ...
  */
  function scores(argument, message, sender) {
    scores.add = function() {
      self.postThought_('Adding scores.');
      ...
      var addScores_ = function() {
        ...
        function sheetReady(err, spreadsheet) {
          ...
          spreadsheet.receive(function(err, rows, info) {
            ...
            spreadsheet.send(function(err) {
              if(err) console.log(err);
                self.postThought_('Scores added!');
            });
          });
        });
      };
    ...
    scores.lvp = function() {
      ...
      self.postThought_('Current LVP: '+leastValuablePlayer.toStats());
    };
    scores.mvp = function() {
      ...
      self.postThought_('Current MVP: '+mostValuablePlayer.toStats());
    };
    scores.of = function() {
      _.forEach(all_players_, function (player) {
      if (player.name==message)
        self.postThought_('Stats: '+player.toStats());
    });
    };
    scores.streak = function (player) {
      ...
      self.postThought_(player.name+' is '+streak+' with ('+mod+streakN+')');
    };
    scores.update = function() {
      cachePlayers_();
      self.postThought_('Updating season scores');
    };
    ...
    if (argument)
      ...
      self.postThought_('What about the scores '+sender+'?');
  };
  this.scores = scores;
  ...
  */
  function jk(argument, message, sender) {
    jk.butnotreally = function() {
      self.postThought_('trololjk');
    };
    if (argument)
      this.jk[argument]();
    else {
      self.postThought_('jk');
  ...
  function suck(argument, message, sender) {
    suck.my = function() {
      ...
      if (sender=='Nico Mendoza'||sender=='Nico')
        self.postThought_('yeah suck '+sender+'\'s tiny '+message+'!');
      else
        self.postThought_('yeah suck '+sender+'\'s '+message+'!');
    };
    suck.his = function() {
      self.postThought_('yeah suck his '+message+'! ');
      self.postThought_('wait, what?');
    };
    if (argument)
      suck[argument]();
    else
      self.postThought_('What about sucking '+sender+'\'s '+message+'?');
  };
  this.suck = suck;
}

// module.exports = commands.activate;
exports.activate = commands.activate;