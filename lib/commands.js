...
commands.prototype = {
  ...
  */
  saveTeamShitData : function () {
    if (!config.saving) return console.log("Not saving S3.");
    ...
    if (!config.localSave) {
      ...
      fs.writeFile(localSeasonsPath, JSON.stringify({seasons:league.seasons},null,4), function (err) {
        if (err) return think_(err);
  ...
  */
  ...
  // season stuff
  season : function(argument, message, sender, callback) {

    function fresh() {
      league.fresh({label:message});
      return callback(null,"Season Created");
    }
    this.season.fresh = fresh;

    if (argument)
      this.season[argument]();
    else
