...
var CurrentLeague = require('./league.js').getCurrentLeague();
  ...
  (function loadTeamShitData() {
    ...
    if (!config.localLoad)
      s3.getObject(s3_params, function(err, data) {
        ...
        console.log('Team Shit (aws) Loaded');
      });
    else 
      fs.readFile(localTeamShitPath, function read(err, data) {
        ...
        console.log('Team Shit (local) Loaded');
      });
  })();
        ...
        spreadsheet.receive(function(err, rows, info) {
          ...
          _.forEach(rows, function(row) {
              if (once) {
                ...
                CurrentLeague.getCurrentSeason().setPlayers(all_players_temp);
