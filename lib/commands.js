...
bot = new bot();
  ...
  */
  function bottle(argument, message, sender) {
      ...
      function sheetReady(err, spreadsheet) {
        ...
        spreadsheet.receive(function(err, rows, info) {
          ...
          spreadsheet.send(function(err) {
            ...
            bot.postThought_('Bottle Duty: '+person);
          });
        });
      });
    };
    ...
    bottle.what = function() {
      ...
      bot.postThought_(bottles[Math.random(0,bottles.length)]);
    };
    if (argument)
      bottle[argument]();
    else
      bot.postThought_('bottle fail');
  };
  this.bottle = bottle;

  // Runs the cool guy thing
  function coolguy() {
    bot.postThought_(cool());
  };
  ...
  */
  function scores(argument, message, sender) {
    scores.add = function() {
      bot.postThought_('Adding scores.');
    ...
    scores.lvp = function() {
      ...
      bot.postThought_('Current LVP: '+leastValuablePlayer.toStats());
    };
    scores.mvp = function() {
      ...
      bot.postThought_('Current MVP: '+mostValuablePlayer.toStats());
    };
    scores.of = function() {
      _.forEach(config.all_players_, function (player) {
      if (player.name==message)
        bot.postThought_('Stats: '+player.toStats());
    });
    };
    scores.streak = function (player) {
      ...
      bot.postThought_(player.name+' is '+streak+' with ('+mod+streakN+')');
    };
    scores.update = function() {
      ...
      function sheetReady(err, spreadsheet) {
        ...
        spreadsheet.receive(function(err, rows, info) {
          ...
          bot.postThought_('Season scores updated');
        });
      });
    ...
    if (argument)
      ...
      bot.postThought_('What about the scores '+sender+'?');
  };
  this.scores = scores;
  ...
  function suck(argument, message, sender) {
    suck.my = function() {
      ...
      if (sender=='Nico Mendoza'||sender=='Nico')
        bot.postThought_('yeah suck '+sender+'\'s tiny '+message+'!');
      else
        bot.postThought_('yeah suck '+sender+'\'s '+message+'!');
    };
    suck.his = function() {
      bot.postThought_('yeah suck his '+message+'! ');
      bot.postThought_('wait, what?');
    };
    if (argument)
      suck[argument]();
    else
      bot.postThought_('What about sucking '+sender+'\'s '+message+'?');
  };
  this.suck = suck;
}
