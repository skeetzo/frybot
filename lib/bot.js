...
bot.prototype = {
  ...
  activate : function(what) {
    // commands.activate(what, this.say);
    ...
    if (typeof commands[command] === "function" ) {
      ...
      commands[command](request, this_.say);
      // likeMessage_(sender);
    }
    else
      think_('No command found');
  ...
  */
  boot : function() {
    ...
    commands.load(function onLoad(err) {
      ...
      self.activate({command: "scores",argument:"boot",name:config.botName});
      if (config.testing) 
  ...
  */
  post : function() {
    ...
    if (request.text.search(config.commandsRegex)!=-1) {
      ...
      this_.activate(request, this_.say);
    }
  ...
  */
  say : function(thought) {
    ...
    this_.thoughts.push(thought);
    clearInterval(this_.postman);
    this_.postman = setInterval(function() {
      this_.postMessage(this_.thoughts.shift());
      if (this_.thoughts.length===0) clearInterval(this_.postman);
      // self.postMaster();
    }, config.responseTime);
  },
