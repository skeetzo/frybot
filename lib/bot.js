...
var bot = function() {
  ...
  bot.prototype.boot = function() {
    ...
    if (config.port<=3000||config.port>=3050) // not within localhost port range
      setTimeout(function() {commands.activate(update)},15000);
    if (config.testing) 
      setTimeout(function() {self_.test();},20000);
  };

  commands.events.on('commands loaded', function() {
    self_.boot();
  })
  ...
  (function cronJobber() {
    ...
    */
    var pregameJob = new CronJob({
      cronTime: '00 00 14 * * 2',
        onTick: function() {
          commands.activate({command:'pregame'});
        },
        start: true,
    ...
    var afterpartyJob = new CronJob({
      cronTime: '00 00 2 * * 3',
        onTick: function() {
          ...
          commands.activate(update);
          commands.activate({command:'bottle',argument:'next'});
...
function think_(what) {
  if (config.debugging) console.log(config.name+': '+what);
}