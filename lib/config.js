...
// Misc
config.port = Number(process.env.PORT || 3004);
...
function setup() {
  ...
  if (!config.debugging) {
    config.name = "Frybot";
    config.GroupMe_group_ID = localConfig.GroupMe_Group_ID  || process.env.GroupMe_Group_ID;
    config.botID = localConfig.GroupMe_bot_ID || process.env.GroupMe_bot_ID;
    config.GroupMe_group_name = '#ItIs!WhatItIs!';
  }
  else {
    config.name = "Naivi";
    config.GroupMeID = localConfig.AWS_ACCESS_KEY || process.env.Naivi_GroupMe_Group_ID;
    config.botID = localConfig.AWS_ACCESS_KEY || process.env.Naivi_GroupMe_bot_ID;
    config.GroupMe_group_name = 'Talking to Myself';
  }

  // setTimeout(function() {
    events.emit('config loaded');
  // },10000);
