var config = {};

config.port = Number(process.env.PORT || 3000);
config.name = "Scytalia";

config.GroupMe_AccessToken = "2f738e5005bc0133e1287ef6bffc9e1d";
config.ItIsWhatItIs_GroupMeID = "14734775";
config.botID = "6c15e36d3a1ac9b72b3cd3049d";

config.minimumReadyPlayers = 1;
config.readyTimerDelay = 10000;
config.talkingToMyself = false;


config.ItIsWhatItIs_serviceEmail = "615638101068-ddthvbjttd2076flaqi1rm54divhpqvk@developer.gserviceaccount.com";


module.exports = config;